...
void trustpilot_check(const char* anagram, size_t anagram_size, void* data)
{
    ...
    //Check and output
    bool easy   = data_equal(MD5_HASH_SIZE, MD5_HASH_SIZE, hash, ((trustpilot_data_t*)data)->hashes[0]);
    bool medium = data_equal(MD5_HASH_SIZE, MD5_HASH_SIZE, hash, ((trustpilot_data_t*)data)->hashes[1]);
    bool hard   = data_equal(MD5_HASH_SIZE, MD5_HASH_SIZE, hash, ((trustpilot_data_t*)data)->hashes[2]);

    if (easy || medium || hard)
    {
        clock_t t_pre = ((trustpilot_data_t*)data)->t_pre;
...
{
    char hash_hex[3][MD5_HASH_SIZE*2] = {
